...
class ChannelPrice(models.Model):
    """Direct price override — how combo pricing works (no base to discount from)."""

    channel = models.ForeignKey(SalesChannel, on_delete=models.CASCADE, related_name="prices")
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name="channel_prices")
    ...
    def __str__(self):
        return f"{self.product} @ {self.channel}: ৳{self.price}"
