...
from catalog.models import Ingredient, Organization, Outlet, PackDefinition, TrackingMode
...
class ReconcileBackdatedStockInTests(TestCase):
    def setUp(self):
        self.org = Organization.objects.create(name="Test Org", slug="test-org-reconcile")
...
class DuplicateInvoiceConstraintTests(TestCase):
    def setUp(self):
        self.org = Organization.objects.create(name="Test Org", slug="test-org-duplicate-invoice")
        self.outlet = Outlet.objects.create(name="Test Outlet", organization=self.org)
