...
class CashSyncOnSubmitTests(APITestCase):
    def setUp(self):
        ...
        self.channel = SalesChannel.objects.create(
            name="Foodi", settlement_type=SettlementType.COLLECTED_AT_OUTLET,
        )
        self.product_a = Product.objects.create(name="Test Item A", requires_preparation=False)
        self.product_b = Product.objects.create(name="Test Item B", requires_preparation=False)
