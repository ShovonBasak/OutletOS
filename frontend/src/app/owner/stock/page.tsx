...
export default function StockLevels() {
  ...
  const totalValue = useMemo(() => {
    let sum = 0;
    for (const r of raw) {
      if (r.cost_per_base_unit) {
        sum += parseFloat(r.quantity_available) * parseFloat(r.cost_per_base_unit);
      }
    }
    return sum;
  }, [raw]);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          className="shrink-0 font-mono text-[10px] text-leaf-deep underline"
          onClick={load}
          disabled={loading}
        >
          {loading ? "…" : "Refresh"}
      ...
      {totalValue > 0 && (
        <div className="ticket flex items-center justify-between py-2.5">
          ...
          <p className="font-mono text-[15px] font-bold text-ink">
            {bdt(String(totalValue.toFixed(2)))}
          </p>
        </div>
      )}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {filteredRaw.length === 0 && (
            <p className="font-mono text-xs text-ink-soft">
              {q ? "No ingredients match." : "No raw stock on hand."}
            </p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {filteredReady.length === 0 && (
            <p className="font-mono text-xs text-ink-soft">
              {q ? "No products match." : "Nothing prepared yet."}
            </p>
          )}
        </div>
      </div>
