...
**No `selling_price` field here anymore** — price-versioned the same way `PackDefinition` already handles purchase cost. See `ProductPrice` right below; "current price" is always the active row there, not a field on `Product` itself.

### ProductPrice (price-versioned) — the base/walk-in selling price
Purchase cost was already versioned via `PackDefinition` (effective_from/effective_to) — this was the one price in the whole system that wasn't, which made it the odd one out. Same pattern, same reasons: schedule a price change in advance, keep a real audit trail of what changed and when, and never have to wonder what a product actually cost on a given past date.
| Field | Type | Notes |
|---|---|---|
| id | PK | |
| product_id | FK → Product | |
| price | decimal | base/walk-in price; for a combo this is its own bundle price, not a sum of parts |
| effective_from | date | can be set in the future to schedule a change in advance |
| effective_to | date, nullable | null = currently active |
| changed_by | FK → User | who made the change — pricing is an owner action |
| note | text, nullable | optional context, e.g. "cost increase from supplier" |

**Resolution:** "current selling price" for a product = the `ProductPrice` row where `effective_from ≤ today ≤ effective_to (or null)`. This is also the base that `ChannelPrice`/`ChannelPromotion` resolve against (see Sales Channels section) — those were already versioned; now every layer of pricing uses the same pattern consistently.

**What doesn't need to change:** historical reporting was never at risk here — `DailyClosingSalesLine.unit_price` already snapshots the resolved price at the moment of each sale, so past revenue figures were always accurate regardless of what happens to price later. This fix is about giving the *base* price itself a proper history and the ability to schedule ahead, not about fixing broken reports.
...
**Price resolution at time of sale (most specific wins):** `ChannelPrice` override (exact product+channel price, e.g. combo pricing) → `ChannelPromotion` (product+channel, then channel-only, then product-only) applied to the active `ProductPrice.price` → plain active `ProductPrice.price`. Resolved price is snapshotted into `DailyClosingSalesLine.unit_price`, so history stays accurate even as prices/promos change later.
...
20. **Selling price is now versioned, matching purchase cost** — `Product.selling_price` was the one price in the system that wasn't versioned, inconsistent with `PackDefinition` (purchase cost) and `ChannelPrice`/`ChannelPromotion` (channel-specific pricing), which already tracked effective_from/effective_to. New `ProductPrice` entity fixes that: full history, ability to schedule a price change in advance, and `changed_by`/`note` for audit. Historical reporting was never actually at risk — `DailyClosingSalesLine.unit_price` already snapshots the resolved price per sale — this was about giving the base price itself a proper history, not fixing broken reports.
